...
**Round 2 (SHIPPED): audio/data** — the mature scene layout
(BootDreams-era convention; also what mkdcdisc emits today:
dreamcast.wiki "Creating a bootable Dreamcast disc"). Session 1 =
silent audio track (generated by cdi4dc itself); session 2 = one
mode-2 data track at **LBA 11702** = our ISO (`mkisofs -C 0,11702` —
...
IP+PVD boot track in session 2 (citation:
`tools/img4dc/cdi4dc/src/cdidata.c` `write_data_header_boot_track`).
Booted in Flycast + real BIOS (`cdi/boot-smoke2`) but **died on real
hardware via GDEMU** — see §Hardware round 1 below. cdi4dc's own
banner calls this mode beta; GDEMU data/data-CDI incompatibilities are
a documented class (dreamcast-talk t=16956: a data/data CDI can boot
burned on a real DC yet not boot on GDEMU).
...
region past the FS is not a BIOS file load and stays plain. The
descramble is keyed to MEDIA TYPE, not track layout: the boot ROM
descrambles `1ST_READ.BIN` whenever it boots CD media, GD boots load
plain (dreamcast.wiki/Scrambling; fabiensanglard.net/dreamcast_hacking
— "when the console detected a CD-ROM, the BOOTROM loaded IP.BIN
normally but scrambled 1ST_READ"; corroborated by dcload's own build
split, §dcload-serial). Empirical evidence — A/B leg pair against the
real BIOS (`dc_boot.bin` present in Flycast's data dir, `UseReios =
no`) in the instrumented Flycast, round-1 data/data geometry
...
### Hardware round 1 — GDEMU, data/data image FAILED (operator, 2026-09-24, no capture)

Round-1 `disc.cdi` (data/data, scrambled) via GDEMU: GDmenu lists and
launches it, license screen WITH our NAOMI MR logo renders (= session-2
IP.BIN served and executed on silicon), then ~5 s black (the bootstrap
loading the 2.5 MB binary), then **console hard-resets back to
GDmenu** — no loader splash, ever. The splash is the loader's first
visible act on hardware (phase 6), so the binary never executed sanely.
Same image, same scramble state, full boot in Flycast + real BIOS
(`cdi/boot-smoke2`) — Flycast's and GDEMU's presentations of this
data/data CDI diverge somewhere after IP.BIN load (descramble decision
inputs or track/pregap byte mapping). GDEMU is closed firmware — not
introspectable; rather than characterize the quirk, the mastering moved
to the audio/data layout every proven tool ships (round 2 above).
**Standing lesson, same class as the emulator-vs-hardware rule: a
Flycast PASS on a CDI variant is not a GDEMU PASS, and neither is a
burned-disc PASS — three distinct presentations of the same image.**

| leg | build | verdict |
|-----|-------|---------|
| *(hw-cdi-round1 — no capture)* | round-1 data/data, scrambled, GDEMU | operator report verbatim above; FAILED |
| `cdi/boot-smoke3` | round-2 audio/data, scrambled (the shipped config) | full boot ladder on the new geometry: loader MMUCRWR `pc=8c01083a`, game takeover `MMUCRWR pc=8c02d630`, **16,880 TAEND frames** (~7 min — the kill misfired, see below, so the leg ran long: a free soak), 0 `System reset`, 0 `SHIMERR`, cart streaming to GDDMA `fad=0001ba4e` = cart +204 MB. Emulator PASS for audio/data @ FAD 13644. |

Leg-harness lesson (cost this leg its bounded window): inside the
harness's background zsh the one-call pattern's
...
stamps PVD creation timestamps, so every remaster moves the .cdi md5
