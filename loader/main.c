...
#include "naomi_crc.h"          /* T9 task 6: menu record -> EEPROM game-area poke */
...
static void preset_note(void) {
    static const char *rows[] = {
        ...
        "release zip to the ROOT of your SD card,",
        "then launch the game again. The settings",
        "apply automatically.",
        "",
...
#define LOADER_MENUDIAG 0   /* T9 task 6: force-feeds a known settings record
                             * through the EEPROM poke path with no operator
                             * -- the unattended byte leg (MENUDIAG=1, top
    ...
    *(uint32 *)(STAGE_SHIM + (GD_STACK_BOTTOM - SHIM_BASE)) = GD_STACK_CANARY;
...
#if LOADER_MENU || LOADER_MENUDIAG
#if LOADER_MENUDIAG
     ...
     * area +0x24..0x4B: CRC headers x2 + record x2). The shim's live EEPROM
     * path (shims/src/main.c case 0x03/0x0b, docs/kb/phase4-conversion.md
     * "EEPROM -- a RAM copy, session-only") lazily copies this baked blob
     * into its own RAM buffer on first access, so this staged poke IS the
     * whole write path. Self-check + pristine fallback: a bug here can only
     * ever yield the baked defaults, never a corrupt image.
     *
     * ADAPTED from the task brief: EEPROM_IMG_ADDR resolves to `mie_sub03`
     * (the generated MIE sub-0x03 reply blob), not a standalone `eeprom_img`
